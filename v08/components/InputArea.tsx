...
export const InputArea: React.FC<InputAreaProps> = ({ 
  ...
  // Particles for Checkout - Updated to include velocity
  const [particles, setParticles] = useState<{id: number, x: number, y: number, angle: number, emoji: string, velocity: number}[]>([]);
  
  // Mouse tracking for "Liquid Light" button
  ...
  const handleCustomInputSubmit = (e: React.FormEvent) => {
      ...
      if(question.multiSelect) {
          // Explicitly combine selection with custom input for immediate submit
          const finalValues = [...selectedOptions, customInputValue];
          
          // Generate nice label
          const selectedLabels = question.options
             ?.filter(opt => selectedOptions.includes(opt.value))
             .map(opt => opt.label) || [];
          selectedLabels.push(customInputValue);
          
          onAnswer(finalValues, selectedLabels.join(', '));
      } else {
  ...
  const handleMultiSubmit = () => {
    const trimmedCustom = customInputValue.trim();
    if (selectedOptions.length === 0 && !trimmedCustom) return;
    
    playClick();
    
    // Combine selected options and custom value if present
    const finalValues = [...selectedOptions];
    if (trimmedCustom) {
        finalValues.push(trimmedCustom);
    }
    ...
    if (trimmedCustom) {
        labels.push(trimmedCustom);
    }

    onAnswer(finalValues, labels.join(', '));
  };
  ...
  const handleButtonMouseMove = ({ currentTarget, clientX, clientY }: React.MouseEvent) => {
    const { left, top } = currentTarget.getBoundingClientRect();
    btnMouseX.set(clientX - left);
    btnMouseY.set(clientY - top);
  };
  ...
  if (question.id === 'select_plan') {
    ...
    const handleCheckoutClick = (e: React.MouseEvent) => {
        ...
        // Massive Explosion: 50 particles instead of 16
        const newParticles = Array.from({ length: 50 }).map((_, i) => ({
            ...
            velocity: 200 + Math.random() * 400 // Much faster/further spread (200-600px)
        }));
    ...
    return (
        ...
        >
            ...
            <div 
                ...
                style={{ 
                    backgroundColor: isPro ? '#0a0a0a' : 'white',
                    borderColor: isPro ? '#000000' : theme.colors.secondary,
            ...
            >
                ...
                {isPro && (
                    <div className="absolute inset-0 opacity-20 pointer-events-none overflow-hidden rounded-[2.5rem]">
                         <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_-20%,_rgba(255,255,255,0.1),_transparent)]" />
                         <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,_transparent_1px)] bg-[size:100%_4px]" />
                    </div>
                )}
            ...
            {/* --- POWER STATS (NEW) --- */}
            <div className="w-full grid grid-cols-2 gap-3 md:gap-6 mt-4">
                <div className="relative group overflow-hidden bg-white rounded-[20px] p-4 border-2 border-gray-100 shadow-lg flex flex-col items-center justify-center hover:border-blue-500 transition-colors">
                    <div className="absolute top-0 left-0 w-full h-1 bg-blue-500" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Daily Output</span>
                    <div className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight leading-none">{fmtNum(dailyEmails)}</div>
                    <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full mt-2">EMAILS / DAY</span>
                </div>

                <div className="relative group overflow-hidden bg-white rounded-[20px] p-4 border-2 border-gray-100 shadow-lg flex flex-col items-center justify-center hover:border-purple-500 transition-colors">
                    <div className="absolute top-0 left-0 w-full h-1 bg-purple-500" />
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Monthly Capacity</span>
                    <div className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight leading-none">{fmtNum(monthlyEmails)}</div>
                    <span className="text-[10px] font-bold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-full mt-2">EMAILS / MONTH</span>
                </div>
            </div>

            {/* --- HOLOGRAPHIC GLASS CTA BUTTON --- */}
            <motion.button
                whileHover={{ scale: 1.03 }} // Increased punch
                whileTap={{ scale: 0.96 }}
                onMouseMove={handleButtonMouseMove}
                onClick={handleCheckoutClick}
                className="group relative w-full mt-6"
            >
                {/* 1. ROTATING BORDER STROKE */}
                <div className="absolute -inset-[3px] rounded-full overflow-hidden z-0">
                    <div className="absolute inset-[-50%] bg-[conic-gradient(from_0deg,transparent_0_300deg,#facc15_330deg,#22c55e_360deg)] animate-[spin_2s_linear_infinite]" />
                </div>
                
                {/* 2. OUTER GLOW (Pulsing Green/Gold) */}
                <div className="absolute -inset-1 rounded-full bg-gradient-to-r from-emerald-600/60 via-yellow-500/50 to-emerald-600/60 blur-xl opacity-20 group-hover:opacity-100 transition-opacity duration-500 z-0 animate-pulse" />

                {/* 3. BUTTON CONTAINER (Magic Glass Orb) */}
                <div className="relative w-full rounded-full z-10 p-[2px] bg-gradient-to-b from-white/30 to-white/5 shadow-2xl backdrop-blur-sm">
                     {/* ORB BODY */}
                     <div className="relative w-full h-full rounded-full flex items-center justify-between px-6 py-5 md:px-8 md:py-6 overflow-hidden transition-all duration-700 bg-black group-hover:bg-[#021f15]">
                        
                        {/* AMBIENT ORB DEPTH (Static center) */}
                        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,_rgba(22,163,74,0.3),_transparent_70%)] opacity-50 group-hover:opacity-100 transition-opacity duration-500" />
                        
                        {/* ACTIVE MOUSE LIGHT (Wild Flash) */}
                        <motion.div 
                            className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-75"
                            style={{
                                background: useMotionTemplate`radial-gradient(180px circle at ${btnMouseX}px ${btnMouseY}px, rgba(250, 204, 21, 0.5), rgba(34, 197, 94, 0.4), transparent 60%)`,
                                mixBlendMode: 'hard-light'
                            }}
                        />

                        {/* GLASS REFLECTIONS & SHADOWS */}
                        {/* Deep Inner Shadow for Roundness */}
                        <div className="absolute inset-0 rounded-full shadow-[inset_0_2px_4px_rgba(255,255,255,0.1),_inset_0_-10px_20px_rgba(0,0,0,0.9)] pointer-events-none" />
                        
                        {/* Top Specular Highlight */}
                        <div className="absolute top-0 left-[20%] right-[20%] h-[40%] bg-gradient-to-b from-white/20 to-transparent rounded-[100%] blur-[4px] pointer-events-none" />

                        {/* Content Layer */}
                        <div className="relative z-10 flex flex-col items-start pointer-events-none">
                             <div className="flex items-center gap-2 mb-1">
                                <span className={`bg-gradient-to-r ${isPro ? 'from-yellow-300 via-orange-300 to-yellow-300' : 'from-emerald-300 via-teal-300 to-emerald-300'} text-black text-[9px] font-black uppercase px-2 py-0.5 rounded-[4px] shadow-[0_0_15px_rgba(255,255,255,0.6)] tracking-wider animate-pulse`}>
                                    {isPro ? 'FOUNDERS ACCESS' : 'INSTANT SETUP'}
                                </span>
                            </div>
                            
                            {/* Text with Shimmer Animation */}
                            <div className="relative">
                                <div className="text-white font-black text-2xl md:text-3xl uppercase tracking-tighter italic flex items-center gap-2 drop-shadow-lg group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-yellow-100 group-hover:via-white group-hover:to-yellow-100 transition-all duration-300">
                                    LAUNCH CAMPAIGN <Rocket size={24} className={`transition-transform duration-300 group-hover:translate-x-1 group-hover:-translate-y-1 ${isPro ? "text-yellow-400 fill-yellow-400" : "text-emerald-400 fill-emerald-400"}`} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="relative z-10 flex items-center gap-6 pointer-events-none">
                            <div className="text-right hidden md:block">
                                <div className="text-white font-black text-3xl md:text-4xl leading-none tracking-tighter drop-shadow-md group-hover:text-emerald-100 transition-colors">
                                    {fmt(totalFirstMonth)}
                                </div>
                            </div>
                            
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_25px_rgba(34,197,94,0.6)] group-hover:border-emerald-400 ${isPro ? 'bg-yellow-400 border-yellow-200 text-black' : 'bg-white border-white text-black'}`}>
                                <ArrowRight size={24} strokeWidth={3} className="group-hover:-rotate-12 transition-transform" />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {particles.map(p => (
                    <motion.div
                        ...
                        animate={{ 
                            x: p.x + (Math.cos(p.angle * Math.PI / 180) * p.velocity), 
                            y: p.y + (Math.sin(p.angle * Math.PI / 180) * p.velocity) - 150, // Move up more
                            opacity: 0, 
                            scale: 2.5, // Larger end scale
                            rotate: Math.random() * 720 - 360 // More rotation
                        }}
                        transition={{ duration: 1.5, ease: "easeOut" }} // Slower duration for larger distance
                        className="absolute text-5xl z-50 pointer-events-none drop-shadow-md" // Larger base font
                    >
                        {p.emoji}
                    </motion.div>
                ))}
      ...
      <AnimatePresence mode='wait'>
        {question.type === 'choice' && question.options ? (
          ...
          >
            ...
            <AnimatePresence>
              {question.multiSelect && (selectedOptions.length > 0 || customInputValue.trim().length > 0) && (
                <motion.div
                   layout
                ...
                >
                  ...
                  >
                    Continue ({selectedOptions.length + (customInputValue.trim() ? 1 : 0)}) <ArrowRight size={20} />
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
