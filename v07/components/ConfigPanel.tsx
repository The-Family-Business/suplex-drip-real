...
import { Copy, RefreshCw, Eye, EyeOff, Zap, Box, Wind, FileCode, FileJson, Download } from 'lucide-react';
...
export const ConfigPanel: React.FC<ConfigPanelProps> = ({ theme, setTheme, questions, setQuestions, onExport, onReset }) => {
  ...
  const handleExportThemeCSS = () => {
    const cssLines = [
      ...
      `  --color-bot-bubble-bg: ${theme.colors.botBubbleBg};`,
      `  --color-bot-bubble-text: ${theme.colors.botBubbleText};`,
      `  --color-user-bubble-bg: ${theme.colors.userBubbleBg};`,
      `  --color-user-bubble-text: ${theme.colors.userBubbleText};`,
      `  --color-ai-suggestion-bg: ${theme.colors.aiSuggestionBg};`,
      `  --color-ai-suggestion-text: ${theme.colors.aiSuggestionText};`,
      `  --color-ai-suggestion-border: ${theme.colors.aiSuggestionBorder};`,
      '',
      '  /* Sizing (px) */',
      `  --size-font-base: ${theme.sizing.baseFontSize}px;`,
      `  --size-font-button: ${theme.sizing.buttonFontSize}px;`,
      `  --size-radius-card: ${theme.sizing.cardBorderRadius}px;`,
      `  --size-padding-card: ${theme.sizing.cardPadding}px;`,
      `  --size-border-width: ${theme.sizing.borderWidth}px;`,
      `  --size-radius-bubble: ${theme.sizing.bubbleBorderRadius}px;`,
      `  --size-button-height: ${theme.sizing.minButtonHeight}px;`,
      `  --size-font-ai: ${theme.sizing.aiSuggestionFontSize}px;`,
      '',
      '  /* Physics (Framer Motion) */',
      `  --physics-stiffness: ${theme.physics.stiffness};`,
      `  --physics-damping: ${theme.physics.damping};`,
      '}'
    ];
    downloadFile('theme-variables.css', cssLines.join('\n'), 'text/css');
  };

  const handleExportQuestionsJSON = () => {
    downloadFile('questions.json', JSON.stringify(questions, null, 2), 'application/json');
  };
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        ...
        {activeTab === 'style' && (
            <>
                ...
                <div className="space-y-4">
                    <h3 className="text-xs font-bold uppercase tracking-widest text-gray-400">AI Suggestions</h3>
                    <div className="grid gap-3">
                        <ColorInput label="Suggestion Bg" value={theme.colors.aiSuggestionBg} onChange={(v) => handleColorChange('aiSuggestionBg', v)} />
                        <ColorInput label="Suggestion Text" value={theme.colors.aiSuggestionText} onChange={(v) => handleColorChange('aiSuggestionText', v)} />
                        <ColorInput label="Suggestion Border" value={theme.colors.aiSuggestionBorder} onChange={(v) => handleColorChange('aiSuggestionBorder', v)} />
                    </div>
                    <RangeInput label="Text Size" value={theme.sizing.aiSuggestionFontSize} min={16} max={48} step={1} onChange={(v) => handleSizingChange('aiSuggestionFontSize', v)} />
                </div>
                ...
                <div className="flex flex-col gap-3 pt-4 pb-8">
                    <h3 className="text-xs font-bold uppercase tracking-widest text-gray-400">Export Options</h3>
                    
                    <div className="grid grid-cols-2 gap-2">
                        <button 
                            onClick={handleExportThemeCSS} 
                            className="flex items-center justify-center gap-2 bg-gray-100 text-gray-700 py-3 rounded-xl hover:bg-gray-200 font-bold text-xs"
                        >
                            <FileCode size={16} /> Theme CSS
                        </button>
                        <button 
                            onClick={handleExportQuestionsJSON} 
                            className="flex items-center justify-center gap-2 bg-gray-100 text-gray-700 py-3 rounded-xl hover:bg-gray-200 font-bold text-xs"
                        >
                            <FileJson size={16} /> Questions JSON
                        </button>
                    </div>

                    <button 
                        onClick={onExport} 
                        className="flex items-center justify-center gap-2 w-full bg-slate-900 text-white py-3 rounded-xl hover:bg-slate-800 font-bold shadow-lg mt-2"
                    >
                        <Download size={16} /> Export User Submission
                    </button>
                    ...
                    <button 
                        onClick={onReset} 
                        className="flex items-center justify-center gap-2 w-full border border-gray-200 text-gray-600 py-3 rounded-xl hover:bg-gray-50 font-semibold"
                    >
        ...
        {activeTab === 'content' && (
            <div className="space-y-4">
                ...
                <div className="space-y-3">
                 ...
                 {/* Duplicate Content Export Actions for convenience */}
                 <div className="pt-4 border-t border-gray-100">
                    <button 
                        onClick={handleExportQuestionsJSON} 
                        className="flex items-center justify-center gap-2 w-full bg-gray-100 text-gray-700 py-3 rounded-xl hover:bg-gray-200 font-bold text-xs"
                    >
                        <FileJson size={16} /> Export Questions JSON
                    </button>
                </div>
            </div>
        )}
