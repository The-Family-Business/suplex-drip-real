...
import { Send, ArrowRight, Check, Plus, Sparkles } from 'lucide-react';
...
export const InputArea: React.FC<InputAreaProps> = ({ question, onAnswer, disabled, theme, aiSuggestion }) => {
      ...
      <AnimatePresence mode='wait'>
        {question.type === 'choice' && question.options ? (
          ...
          >
             {/* AI Suggestion Chip */}
             <AnimatePresence>
                {aiSuggestion && !textValue && (
                    <motion.button
                        initial={{ opacity: 0, y: 10, scale: 0.9 }}
                        animate={{ opacity: 1, y: 0, scale: 1 }}
                        exit={{ opacity: 0, y: 5 }}
                        onClick={handleApplySuggestion}
                        className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/90 backdrop-blur shadow-lg border border-indigo-100 text-indigo-600 font-bold text-sm hover:scale-105 transition-transform origin-bottom-left mb-2 ml-2"
                    >
                        <Sparkles size={16} className="text-amber-400 fill-amber-400" />
                        Suggestion: {aiSuggestion.length > 30 ? aiSuggestion.substring(0, 30) + '...' : aiSuggestion}
                    </motion.button>
                )}
             </AnimatePresence>
